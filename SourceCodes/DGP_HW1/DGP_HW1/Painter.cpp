...
SoSeparator* Painter::drawEdge(Mesh* mesh, int edgeIndex) {
...
SoSeparator* Painter::paintCurvature(Mesh* mesh, vector<int>& vertsToColored)
{
	SoSeparator* res = new SoSeparator();

	SoMaterial* mat = new SoMaterial();

	cout << "Curvature size color: " << vertsToColored.size() << endl;

	for (int i = 0; i < vertsToColored.size(); i++)
	{
		if (vertsToColored[i] == 0)
		{
			mat->diffuseColor.set1Value(i, SbColor(1.0f, 0.0f, 0.0f));			//red
		}
		else if (vertsToColored[i] == 1)
		{
			mat->diffuseColor.set1Value(i, SbColor(0.0f, 1.0f, 0.0f));			//green
		}
		else if (vertsToColored[i] == 2)
		{
...
SoSeparator* Painter::paintSphere(Mesh* mesh, vector<int>& vertsToSpehered)
{
	SoSeparator* res = new SoSeparator();
	...
	for (int i = 0; i < 100; i++)
	{
		...
		SbVec3f tempLoc(mesh->verts[vertsToSpehered[i]]->coords[0], mesh->verts[vertsToSpehered[i]]->coords[1], mesh->verts[vertsToSpehered[i]]->coords[2]);
		transform->translation.setValue(tempLoc);
		...
		{
			mat->diffuseColor.setValue(SbColor(1.0f, 0.0f, 1.0f));			//blue

		}
		sphereSep->addChild(mat);

		// shape
		SoSphere* sphereShape = new SoSphere;
		sphereShape->radius = 1.0f;
		sphereSep->addChild(sphereShape);
